...
{
    public class ToutiaoRepository : BaseRepository<Models.Toutiao>
    {
        ...
        /// <returns></returns>
        public IEnumerable<Toutiao> LoadPageList(string title, int categoryID, string categoryCode, int startNum, int pageSize, out int rowCount)
        {
            rowCount = 0;
            var result = from p in db.Set<Toutiao>()
            ...
            return result.ToList().Select(m => new Toutiao()
            {
                ID = m.ID,
        ...
        public Toutiao GetToutiao(int id)
        {
            var result = from p in db.Set<Toutiao>()
                         join a in db.Set<Category>() on new { CategoryID = p.CategoryID } equals new { CategoryID = a.ID } into a_into
                         from a in a_into.DefaultIfEmpty()
        ...
        public IEnumerable<Toutiao> GetToutiaos(string keyword,int memberID,string categoryCode, string order, int startNum, int pageSize, out int totalcount)
        {
            var result = from p in db.Set<Toutiao>()
                         join a in db.Set<Category>() on new { CategoryID = p.CategoryID } equals new { CategoryID = a.ID } into a_into
                         from a in a_into.DefaultIfEmpty()
